...
export default function AcademyAttendanceDashboard() {
  const [logs, setLogs] = useState(INITIAL_ATTENDANCE)
  ...
  const updateStatus = (id: string, status: string) => {
    setLogs(logs.map(log => log.id === id ? { ...log, status, checkIn: status === "PRESENT" ? "10:00 AM" : "--" } : log))
  }
      ...
      <div className="fixed inset-0 z-50 bg-background flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-border/50">
          ...
          <div className="flex gap-4">
            <button onClick={handleSubmitAttendance} className="px-6 py-3 bg-primary text-primary-foreground font-bold rounded-xl shadow-lg hover:bg-primary/90 transition-all text-lg">
              Submit Attendance
            </button>
        ...
        <div className="flex-1 p-8 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
            {logs.filter(l => l.batch === batchFilter).map(student => (
