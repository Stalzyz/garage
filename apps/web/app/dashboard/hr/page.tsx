...
import { Search, Plus, Filter, Mail, Phone, Calendar, MoreVertical, MapPin, Users, Upload, X, FileText, Printer } from "lucide-react"
...
export default function EmployeeDirectory() {
  ...
  const filtered = employees.filter((emp: any) => {
    const nameMatch = emp.user?.name?.toLowerCase().includes(search.toLowerCase()) || false
          ...
          {filtered.map((emp: any, i: number) => (
            ...
            >
              ...
              <div className="flex flex-col items-center text-center mb-6 relative z-10">
                ...
                <h3 className="text-lg font-bold text-white mb-1 group-hover:text-emerald-400 transition-colors">{emp.user?.name}</h3>
                <p className="text-[10px] font-mono tracking-widest uppercase text-emerald-400/80 bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20">{emp.designation}</p>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="w-full md:w-1/3 bg-white/5 border-r border-white/10 p-8 flex flex-col items-center">
                 ...
                 <h2 className="text-xl font-bold text-white text-center mb-2">{selectedEmployee.user?.name}</h2>
                 <p className="text-[10px] font-mono tracking-widest uppercase text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-md border border-emerald-500/20 mb-8">{selectedEmployee.designation}</p>
                 ...
                 <div className="w-full space-y-4">
                   ...
                   <div className="bg-black/40 border border-white/5 rounded-xl p-4">
                     <p className="text-[9px] font-mono tracking-widest uppercase text-white/40 mb-1">Basic Salary</p>
                     <p className="text-sm font-bold text-white">₹{selectedEmployee.basicSalary?.toLocaleString()}</p>
                   </div>
                   <div className="bg-black/40 border border-white/5 rounded-xl p-4">
                     <p className="text-[9px] font-mono tracking-widest uppercase text-white/40 mb-1">Joining Date</p>
                     <p className="text-sm font-bold text-white">{new Date(selectedEmployee.joinDate).toLocaleDateString()}</p>
                   </div>
                 </div>
              </div>
