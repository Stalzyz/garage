...
export default function ClientProjectDetailPage() {
  ...
  const handleFileUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchApi(`/portal/projects/${id}/files`, {
        method: 'POST',
        body: JSON.stringify({
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
            ...
            <div className="border-t border-white/5 my-4 pt-4 flex flex-col gap-2">
              ...
              >
                <UploadCloud className="w-4 h-4 text-blue-400" /> Upload File
              </button>
              <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="bg-[#14141f] border border-white/10 rounded-3xl w-full max-w-md p-6 relative overflow-hidden">
            ...
            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
              <UploadCloud className="w-5 h-5 text-blue-400" /> Upload Project File
            </h3>
            <form onSubmit={handleFileUploadSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  required
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-500 file:cursor-pointer" 
                />
              </div>
              <button 
                type="submit" 
                disabled={uploadLoading}
                className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-white/10 rounded-xl text-sm font-bold text-white transition-colors flex items-center justify-center gap-2"
              >
